...
emotionRoutes.get('/gradient', async (req: Request, res: Response) => {
  try {
    ...
    if (nextGradient.level > gradient.level) {
      ...
      // 找更高一�?      const allGradients = [0, 1, 2, 3].map((g) => getGradient(g * 100))
...
// ============================================================
// GET /api/v1/emotion/history �?情感历史（用于调�?图表�?// ============================================================
